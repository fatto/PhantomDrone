...
Haptic::Haptic() : start_position(0,0,0), position(0,0,0), force(0,0,0), start_gimbal(0,0,0), gimbal(0,0,0), max_force(0)
{
	hHD = hdInitDevice(HD_DEFAULT_DEVICE);
	testDevice();

	gCallbackHandle = hdScheduleAsynchronous(&Haptic::monitorDevice, this, HD_MAX_SCHEDULER_PRIORITY);
...
Haptic::StatusStruct Haptic::Status()
{
...
//std::array<double, 3> Haptic::Position()
//{
//	std::lock_guard<std::mutex> lock(device_mutex);
//	return std::array<double,3>{position[0], -position[2], position[1]};
//}
//
//std::array<double, 3> Haptic::Displacement()
//{
//	std::lock_guard<std::mutex> lock(device_mutex);
...
void Haptic::Force(std::array<double, 3> f)
{
	...
	std::lock_guard<std::mutex> lock(device_mutex);
	force = hduVector3Dd(f.data());
}

bool Haptic::testDevice()
{
	bool valid = false;
...
HDCallbackCode HDCALLBACK Haptic::monitorDevice(void *pUserData)
{
	Haptic* device = static_cast<Haptic*>(pUserData);
	...
	hdSetDoublev(HD_CURRENT_FORCE, device->force);
